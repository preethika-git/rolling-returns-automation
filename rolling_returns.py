...
# Logging utility (writes console + file)
def make_logger(base_path: Path):
    ...
    def log(msg, err=False):
        ...
        try:
            ...
            pass
        # flush prints so console shows live progress
        print(line, flush=True)
        if err:
            pass
    return log

# Determine base path (works for script and for PyInstaller exe)
...
# Load scheme codes JSON (must be in same folder as exe/script)
...
def monthly_rolling_return(df):
    ...
    return rr  # decimal (e.g., 0.053)

# Main: build results per category
...
def export_to_excel(results: dict, base_path: Path, logger):
    ...
    with pd.ExcelWriter(filename, engine="xlsxwriter") as writer:
        ...
        for category, rows in results.items():
            ...
            # ensure columns order: AMC, Rolling Return - Direct, Rolling Return - Regular
            ...
            # write dataframe starting at row=1,col=1 to leave top-left padding
            df.to_excel(writer, sheet_name=category, index=False, startrow=1, startcol=1)
            worksheet = writer.sheets[category]

            # set padding row & column
            worksheet.set_row(0, padding_row_height)
            worksheet.set_column(0, 0, padding_col_width)

            # rewrite headers with formatting (headers are at row=1 due to startrow=1)
            for col_idx, col_name in enumerate(df.columns, start=1):
                worksheet.write(1, col_idx, col_name, header_fmt)

            # write values with proper formats and compute widths using formatted strings
            col_widths = {col: len(str(col)) for col in df.columns}
            for r_idx, row in enumerate(rows, start=2):  # excel row index (0-based) + start=2 for data rows
                # row is dict with AMC and RR values
                for c_idx, col in enumerate(df.columns, start=1):
                    raw_val = row.get(col)
                    # if Rolling Return column and numeric, write numeric with percent format
                    if col.startswith("Rolling Return"):
                        if raw_val is None or (isinstance(raw_val, float) and (np.isnan(raw_val) or np.isinf(raw_val))):
                            ...
                            # write numeric decimal (0.053 -> 5.30%)
